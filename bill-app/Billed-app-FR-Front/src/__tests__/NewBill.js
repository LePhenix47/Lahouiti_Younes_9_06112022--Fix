...
describe("Given I am connected as an employee", () => {
  ...
  describe("I'm posting a form that is valid", () => {
    test("we fetch the new Bills from the mock using the API with the POST protocol", async () => {
      ...
      console.log(
        "storeFromMock",
        Promise.resolve(storeFromMock.bills().update(newBill))
      );
      const postFunctionTest = jest.spyOn(
        storeFromMock.bills().update(newBill),
        "update"
      );

      await storeFromMock.bills().update(newBill);
      expect(postFunctionTest).toHaveBeenCalledTimes(1);
      expect(postFunctionTest).toHaveBeenLastCalledWith(newBill);
    });
  });
});
